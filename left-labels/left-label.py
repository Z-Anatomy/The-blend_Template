...
def register_keymaps():
    ...
    if not all(i in kc.active.keymaps for i in areas):
        ...
        wm = bpy.context.window_manager
        addon_km = wm.keyconfigs.addon.keymaps.new(
        ...
        ###########
        try:
            km, kmis = get_user_keymap_item(
                'Object Mode', 'object.hide_view_set', multiple_entries=True)
            for default_kmi in kmis:
                addon_kmi = addon_km.keymap_items.new(
                    OBJECT_OT_hide_wrapper.bl_idname, default_kmi.type, default_kmi.value)
                ...
                for prop in default_kmi.properties.__dir__():
                    if not (prop.startswith('_') or prop in {'bl_rna', 'rna_type'}):
                        setattr(addon_kmi.properties, prop, getattr(
                            default_kmi.properties, prop))
            ...
            addon_kmi = addon_km.keymap_items.new(
                OBJECT_OT_hide_wrapper.bl_idname, kmis[0].type, kmis[0].value)
            ...
            km, kmis = get_user_keymap_item(
                'Object Mode', 'object.hide_view_clear', multiple_entries=True)
            for default_kmi in kmis:
                addon_kmi = addon_km.keymap_items.new(
                    OBJECT_OT_hide_view_clear_wrapper.bl_idname, default_kmi.type, default_kmi.value)
                addon_kmi.map_type = default_kmi.map_type
                if hasattr(addon_kmi, 'repeat'):
                    addon_kmi.repeat = default_kmi.repeat
                addon_kmi.any = default_kmi.any
            ...
            ###########
            km, kmis = get_user_keymap_item(
                '3D View', 'view3d.localview', multiple_entries=True)
            for default_kmi in kmis:
                addon_kmi = addon_km.keymap_items.new(
                    OBJECT_OT_local_view_wrapper.bl_idname, default_kmi.type, default_kmi.value)
                addon_kmi.map_type = default_kmi.map_type
                addon_kmi.repeat = default_kmi.repeat
                addon_kmi.any = default_kmi.any
                addon_kmi.shift = default_kmi.shift
            ...
            kmi = addon_km.keymap_items.new(
                OBJECT_OT_make_label.bl_idname, 'FIVE', 'PRESS', shift=True, ctrl=True, alt=False)
            kmi.active = True
            ...
            addon_km = wm.keyconfigs.addon.keymaps.new(
                name='Object Mode', space_type='EMPTY')
            kmi = addon_km.keymap_items.new(
                OBJECT_OT_label_delta.bl_idname, 'FIVE', 'PRESS', shift=True, ctrl=True, alt=True)
            kmi.active = True
            addon_keymaps.append((km, kmi))
            ###########
            # Change Label
            ...
            addon_km = wm.keyconfigs.addon.keymaps.new(
                name='Object Mode', space_type='EMPTY')
            kmi = addon_km.keymap_items.new(
                OBJECT_OT_change_label_wrapper.bl_idname, 'F2', 'PRESS', shift=False, ctrl=False, alt=False)
            kmi.active = True
            addon_keymaps.append((km, kmi))
            ...
            ###########
            kmi = addon_km.keymap_items.new(
                TEXT_OT_wiki_download.bl_idname, 'ZERO', 'PRESS', shift=True, ctrl=True, alt=False)
            kmi.active = True
            addon_keymaps.append((km, kmi))
            ...
            ###########
            kmi = addon_km.keymap_items.new(
                OBJECT_OT_key_color.bl_idname, 'Z', 'PRESS', shift=True, ctrl=False, alt=True)
            kmi.active = True
            addon_keymaps.append((km, kmi))
            ...
            ###########
            kmi = addon_km.keymap_items.new(
                OBJECT_OT_select_parent_children.bl_idname, 'DOWN_ARROW', 'PRESS', shift=False, ctrl=False, alt=False)
            kmi.active = True
            addon_keymaps.append((km, kmi))
...
class OBJECT_OT_label_delta(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context):
        return context.mode == 'OBJECT' and context.object is not None and (context.object.name.endswith('.t') or context.object.name.endswith('.s'))
...
class OBJECT_OT_change_label_wrapper(bpy.types.Operator):
    ...
    custom_label: bpy.props.StringProperty(
        default="Custom Label", name="New Label")

    @classmethod
    def poll(cls, context):
        return context.mode in {'OBJECT'} and context.object is not None and ".t" in context.object.name or '.s' in context.object.name
...
class OBJECT_OT_local_view_wrapper(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        bpy.ops.view3d.localview(
            'INVOKE_DEFAULT', frame_selected=self.frame_selected)
    ...
    def refresh():
        for area in bpy.context.screen.areas:
            if area.type == 'VIEW_3D':
                ...
                # Change here for the left fonts
                for obj in bpy.context.visible_objects:
                    if '.t' in obj.name or obj.name.endswith('...') or '.s' in obj.name:
...
def only_once():
    ...
    if blend_name == 'Z-Anatomy-lite.blend':
        ...
        txt_list = [
            text.name for text in bpy.data.texts if text.name not in DO_NOT_DELETE]

        for text_name in txt_list:
            text = bpy.data.texts[text_name]
            bpy.data.texts.remove(text)

        layer_collections = ['.Skeletal system', '.Muscular insertions', '.Joints', '.Muscular system', '.Cardiovascular system', '.Lymphoid organs',
                             '.Nervous system & Sense organs', '.Visceral systems', '.Regions of human body', '.Reference lines, planes & movements']
        with bpy.data.libraries.load(main_blend, relative=True) as (data_from, data_to):
            ...
            files = [{'name': txt_name}
                     for txt_name in data_from.texts if txt_name != "z-anatomy.py"]
        bpy.ops.wm.append(directory=main_blend+"/Text/",
                          files=files, do_reuse_local_id=True, link=True)
...
def label_group_checkbox_update(*args):
    ...
    if not bpy.context.scene.zanatomy.enable_group_labels:
        for ob in (c for c in bpy.context.scene.objects if c.name.endswith('.g') or (c.name.endswith('.j') or (c.name.endswith('.i')) and c.parent and c.parent.name.endswith('.g'))):
            ob.hide_set(True)
    else:
        for ob in (o for o in set(bpy.context.scene.objects) - set(active_object.users_collection[0].all_objects) if o.name.endswith('.g')):
            ...
            for child in (c for c in bpy.context.scene.objects if c.parent == ob and c.name.endswith('.j') and c.name.endswith('.i')):
                child.hide_set(True)
        ...
        for ob in (o for o in active_object.users_collection[0].all_objects if not o.visible_get() and o.name.endswith('.g')):
            ...
            # for some reason it is 10x faster to iterate over all visible objects to find the child than to iterate over small subset of children...
            for child in (c for c in bpy.context.scene.objects if c.parent == ob and not c.visible_get() and c.name.endswith('.j') and c.name.endswith('.i')):
...
def msgbus_callback(*args):
    ...
    if ".t" in active_object.name or ".st" in active_object.name or ".s" in active_object.name:
        return
    ...
    if not bpy.context.scene.zanatomy.enable_group_labels or active_object.users_collection[0] is bpy.context.view_layer.layer_collection.collection:
        ...
        for ob in (o for o in set(bpy.context.visible_objects) - set(active_object.users_collection[0].all_objects) if o.name.endswith('.g')):
            ...
            # for child in (c for c in ob.children if c.visible_get() and c.name.endswith('.j')):
            # for some reason it is 10x faster to iterate over all visible objects to find the child than to iterate over small subset of children...
            for child in (c for c in bpy.context.visible_objects if c.parent == ob and c.name.endswith('.j') and c.name.endswith('.i')):
                child.hide_set(True)
    ...
    for ob in (o for o in bpy.context.visible_objects if ".t" in o.name or '.s' in o.name):
...
class ZANATOMY_PT_Xsection(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if obj and 'Cross-section-X' in obj:
            ...
            emboss, depress = (
                True, True) if obj['Cross-section-X'] else (True, False)
            op = sub.operator("object.cross_section",
                              emboss=emboss, depress=depress, text='X')
            op.axis = 'X'
            op.enable = not obj['Cross-section-X']

            emboss, depress = (
                True, True) if obj['Cross-section-Y'] else (True, False)
            op = sub.operator("object.cross_section",
                              emboss=emboss, depress=depress, text='Y')
            op.axis = 'Y'
            op.enable = not obj['Cross-section-Y']

            emboss, depress = (
                True, True) if obj['Cross-section-Z'] else (True, False)
            op = sub.operator("object.cross_section",
                              emboss=emboss, depress=depress, text='Z')
            op.axis = 'Z'
            ...
            emboss, depress = (
                True, True) if obj['Cross-section-X-inverse'] else (True, False)
            op = sub.operator("object.cross_section", emboss=emboss,
                              depress=depress, text='', icon='ARROW_LEFTRIGHT')
            op.axis = 'X'
            op.enable = obj['Cross-section-X']
            op.invert = not obj['Cross-section-X-inverse']

            emboss, depress = (
                True, True) if obj['Cross-section-Y-inverse'] else (True, False)
            op = sub.operator("object.cross_section", emboss=emboss,
                              depress=depress, text='', icon='ARROW_LEFTRIGHT')
            op.axis = 'Y'
            ...
            emboss, depress = (
                True, True) if obj['Cross-section-Z-inverse'] else (True, False)
            op = sub.operator("object.cross_section", emboss=emboss,
                              depress=depress, text='', icon='ARROW_LEFTRIGHT')
            op.axis = 'Z'
        ...
        for collection in (c for c in bpy.data.collections if 'Cross-section-X' in c):
            ...
            sub = box.grid_flow(row_major=True, columns=3, align=True)
            emboss, depress = (
                True, True) if collection['Cross-section-X'] else (True, False)
            op = sub.operator("collection.cross_section",
                              emboss=emboss, depress=depress, text='X')
            ...
            emboss, depress = (
                True, True) if collection['Cross-section-Y'] else (True, False)
            op = sub.operator("collection.cross_section",
                              emboss=emboss, depress=depress, text='Y')
            op.collection_name = collection.name
            op.axis = 'Y'
            ...
            emboss, depress = (
                True, True) if collection['Cross-section-Z'] else (True, False)
            op = sub.operator("collection.cross_section",
                              emboss=emboss, depress=depress, text='Z')
            op.collection_name = collection.name
            op.axis = 'Z'
            ...
            emboss, depress = (
                True, True) if collection['Cross-section-X-inverse'] else (True, False)
            op = sub.operator("collection.cross_section", emboss=emboss,
                              depress=depress, text='', icon='ARROW_LEFTRIGHT')
            op.collection_name = collection.name
            op.axis = 'X'
            ...
            emboss, depress = (
                True, True) if collection['Cross-section-Y-inverse'] else (True, False)
            op = sub.operator("collection.cross_section", emboss=emboss,
                              depress=depress, text='', icon='ARROW_LEFTRIGHT')
            op.collection_name = collection.name
            op.axis = 'Y'
            ...
            emboss, depress = (
                True, True) if collection['Cross-section-Z-inverse'] else (True, False)
            op = sub.operator("collection.cross_section", emboss=emboss,
                              depress=depress, text='', icon='ARROW_LEFTRIGHT')
            op.collection_name = collection.name
            op.axis = 'Z'
    ...
    def draw(self, context):
        ...
        for collection in layer_collections:
            if collection not in context.scene.collection.children:
                col.operator('layer.append_layer',
                             text=collection).collection = collection
            else:
                col.operator('layer.remove_layer', text=collection,
                             depress=True).collection = collection
...
class LAYERS_OT_add_layer(bpy.types.Operator):
    ...
    def execute(self, context):
        main_blend = os.path.join(os.path.dirname(
            bpy.data.filepath), 'Z-Anatomy.blend')
        ...
        for tex_coord_node in [node for n in bpy.data.node_groups if n.name.startswith('CrossSectionControllerGroup') for node in n.nodes if node.type == 'TEX_COORD']:
            if tex_coord_node.object and re.match(r'.+\.\d\d\d', tex_coord_node.object.name):
                tex_coord_node.object = bpy.data.objects[re.split(
                    r'\.\d\d\d', tex_coord_node.object.name)[0]]
...
class ZAnatomyProps(bpy.types.PropertyGroup):
    enable_group_labels: bpy.props.BoolProperty(
        default=True, name="Enable Group Labels", update=lambda self, context: label_group_checkbox_update())
    ...
    def key_color_func(self, context):
        if '4: Muscular system' in bpy.data.collections:
            ...
            for ob in [c for col in bpy.data.collections for c in col.all_objects if c.type == 'CURVE' and 'key_color' in c]:
                ...
                ob.update_tag(refresh={'OBJECT'})
        context.area.tag_redraw()
    key_color: bpy.props.BoolProperty(
        default=False, name="Key Color", update=key_color_func)
    ...
    def comic_shader_func(self, context):
        ...
        for ob in bpy.data.objects:
            if 'comic_shader' in ob:
                ...
                ob.update_tag(refresh={'OBJECT'})
        context.area.tag_redraw()
    comic_shader: bpy.props.BoolProperty(
        default=False, name="Comic Shader", update=comic_shader_func)

    def background_color_func(self, context):
        if self.background_color == "GREY":
            for obj in [o for o in bpy.data.objects if re.search(r"(\.st)|(\.t)|(\.s)|(\.g)|$", o.name)]:
                obj[f'background'] = 0
                obj.update_tag(refresh={'OBJECT'})
            bpy.data.worlds["WORLD BACKGROUND"].node_tree.nodes["Background"].inputs[0].default_value = (
                0.029556820169091225, 0.029556820169091225, 0.029556820169091225, 0)
    ...
    def theme_func(self, context):
        ...
        dir_path = os.path.abspath(os.path.join(
            sys.executable, '..', '..', '..', 'scripts', 'presets', 'interface_theme'))

        if self.theme == "DARK":
            theme = "Anatomy_Dark.xml"

            for obj in [o for o in context.selected_objects if re.search(r"(\.st)|(\.t)|(\.s)|(\.g)$", o.name)]:
                obj[f'theme'] = 0
                obj.update_tag(refresh={'OBJECT'})
        else:
