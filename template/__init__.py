    ...
    if not all(i in kc.active.keymaps for i in areas):
        ...
        try:
            if "Z-Anatomy" in bpy.context.window_manager.keyconfigs:
                bpy.context.window_manager.keyconfigs.active = bpy.context.window_manager.keyconfigs[
                    "Z-Anatomy"]
            else:
                bpy.ops.preferences.keyconfig_import(filepath=os.path.join(
                    dirname, "Z-Anatomy.py"), filter_folder=True, filter_text=True, filter_python=True, keep_original=True)
                bpy.context.window_manager.keyconfigs.active = bpy.context.window_manager.keyconfigs[
                    "Z-Anatomy"]
